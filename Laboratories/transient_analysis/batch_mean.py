...
parser.add_argument('--departure', type=int, default=1)
...
# %%
# h = Hyperexp(alpha=1, lambda1=-2/9, lambda2=2/11)
# print(h.mean(), h.var(), h.std())
# %%
# h = Hyperexp(alpha=0.5, lambda1=1/97 * (-2 - 3*np.sqrt(22)), lambda2=1/97 * (3*np.sqrt(22) - 2))
# print(h.mean(), h.var(), h.std())
# %%
    ...
    def arrival(self, time, FES, queue):
        ...
        if self.users == 1:
            ...
            if self.type == "exp":
                ...
                h = Hyperexp(alpha=1, lambda1=-2 / 9, lambda2=2 / 11)  # mean=1, std=10
                service_time = h.rvs()
            else:
                print("ERROR!!")
...
# %%
sim = QueueSimulator(ARRIVAL=.5, DEPARTURE=args.departure, SIZE_LIMIT=args.size_limit,
                     type="exp", batch_size=args.batch_size, n=args.n, p=args.p, seed=args.seed)
# %%
delays_exp = []
conf_int_exp = []
for arrival_rate in arrival_rates:
    sim.set_params(ARRIVAL=arrival_rate, DEPARTURE=args.departure, SIZE_LIMIT=args.size_limit,
                   type="exp", batch_size=args.batch_size, n=args.n, p=args.p, seed=args.seed)
    sim.simulate()
...
for arrival_rate in arrival_rates:
    sim.set_params(ARRIVAL=arrival_rate, DEPARTURE=args.departure, SIZE_LIMIT=args.size_limit,
                   type="det", batch_size=args.batch_size, n=args.n, p=args.p, seed=args.seed)
    sim.simulate()
...
for arrival_rate in arrival_rates:
    sim.set_params(ARRIVAL=arrival_rate, DEPARTURE=args.departure, SIZE_LIMIT=args.size_limit,
                   type="hyper", batch_size=args.batch_size, n=args.n, p=args.p, seed=args.seed)
    sim.simulate()
